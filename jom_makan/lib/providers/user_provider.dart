...
class UserProvider with ChangeNotifier {
  ...
  Future<Map<String, dynamic>> findTopUsers() async {
    ...
    for (var user in _allUsers!) {
      ...
      final postCount =
          int.tryParse(await _userRepository.fetchPostCount(user.userID)) ??
              0; // Default to 0 if parsing fails
      final likeCount =
          int.tryParse(await _userRepository.fetchLikeCount(user.userID)) ??
              0; // Default to 0 if parsing fails
      print(
          'User: ${user.fullName}, Post Count: $postCount, Like Count: $likeCount');
  ...
  Future<void> updateUserData(
    ...
    if (userData != null) {
      await _userRepository.updateUserData(userData!.userID, data, imageFile);
