...
Future<void> signUpRestaurantWithEmail({
  ...
  try {
    ...
    _firebaseUser = await _authRepository.signUpRestaurantWithEmail(
      ...
      intro: intro,
      image: '', // Temporarily set as empty, update after profile image upload
      tags: tags,
    );
    ...
    List<String> menuImageUrls = await _uploadImages(menuImages, restaurantId);
    String profileImageUrl = (await _uploadImages([profileImage], restaurantId)).first;
    ...
    _restaurantData = await _restaurantRepository.getRestaurantData(restaurantId);
  } catch (e) {
 ...
 Future<List<String>> _uploadImages(List<File> images, String restaurantID) async {
